...
if (isset($_SESSION['username'])) {
	...
	if (mysqli_num_rows($get_Pic) > 0) {
		while ($data = mysqli_fetch_assoc($get_Pic)) {
			...
			$stdData = "SELECT e.*, u.* FROM employees e INNER JOIN users u ON e.email = u.email WHERE u.user_id = '$usr_id';";
				...
				<body>
					...
					<div class="main-container">
						<div class="pd-ltr-20 xs-pd-20-10">
							...
							<form action="facultyApproval.php" method="post">
								<ul class="profile-edit-list row">
									<li class="weight-500 col-md-9">
										<h4 class="text-danger h5 mb-20">Faculty Profile</h4>
										...
										<div class="form-group">
											<label for="decision">Hire/Decline Batch</label>
											<select name="decision" id="decision" class="form-control my-3 red-input">

												<?php if ($row['is_approved'] == 1) ?>


												<option value="" disabled selected>Choose category</option>
												<option value="1" <?php if ($row['is_approved'] == 1) echo "selected" ?>>Approved</option>
												<option value="3" <?php if ($row['is_approved'] == 3) echo "selected" ?>>Decline</option>
											</select>
										</div>

										<div class="form-group">
											<label>Salary</label>
											<input class="form-control form-control-lg red-input" type="text" name="up_salary" value="<?php echo $row['salary'] ?>">
										</div>

										<div class="form-group">
											<label>Performance Review</label>
											<textarea class="form-control red-input" name="performance_Review"><?php echo $row['performance_reviews'] ?></textarea>
										</div>
										...
										<!-- <select name="batch_id" id="category_id" class="form-control my-3"> -->
										<!-- <option value="" disabled selected>Choose category</option> -->
										<?php /*
										...
										<div class="form-group mb-0">
											<input type="submit" class="btn btn-warning" name="isApprove" value="Approve Faculty">
